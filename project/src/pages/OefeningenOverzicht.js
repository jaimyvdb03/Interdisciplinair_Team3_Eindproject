...
const OEFENINGEN_STATIC = [
  { id: 1, icon: '📄', status: 'done',   to: '/oefeningen/cv-maken'          },
  { id: 2, icon: '💼', status: 'done',   to: '/oefeningen/sollicitatiebrief'  },
  { id: 3, icon: '👔', status: 'active', to: '/oefeningen/kleding-gedrag'     },
  { id: 4, icon: '⏰', status: 'active', to: '/oefeningen/op-tijd-komen'      },
...
export default function OefeningenOverzicht() {
  ...
  for (let i = 0; i < OEFENINGEN_STATIC.length; i++) {
    ...
    oefeningenCards[i] = (
      ...
      >
        <span className="text-[28px] shrink-0">{oef.icon}</span>
